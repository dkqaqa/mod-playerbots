 ...
 */
 ...
 void PlayerbotHolder::AddPlayerBot(ObjectGuid playerGuid, uint32 masterAccountId)
 {
     ...
     {
         allowed = false;
         out << "[失败]角色 :"<< botName.c_str()<<"不是你的化身。";
     }
     if (masterAccountId && masterPlayer)
     {
         ...
         {
             allowed = false;
             out << "[失败]：当前神念操控化身数量已达上限（ " << sPlayerbotAIConfig->maxAddedBots << ")。";
         }
     }
     if (!allowed)
     {
 ...
 void PlayerbotHolder::HandlePlayerBotLoginCallback(PlayerbotLoginQueryHolder const& holder)
 {
     ...
     {
         // Debug log
         LOG_DEBUG("mod-playerbots", "无法为账户ID加载机器人玩家: {}", botAccountId);
     ...
     {
         LOG_DEBUG("mod-playerbots", "找到主会话，但没有玩家与主账户ID关联: {}", masterAccount);
     }
 ...
 void PlayerbotHolder::LogoutPlayerBot(ObjectGuid guid)
 {
     if (Player* bot = GetPlayerBot(guid))
     {
         ...
         LOG_INFO("playerbots", "机器人 {} 正在登出", bot->GetName().c_str());
     ...
     if (GET_PLAYERBOT_AI(bot))
     {
         if (Player* master = GET_PLAYERBOT_AI(bot)->GetMaster())
         {
             ...
             else if (cmd == "init=auto")
             {
                 ...
                 PlayerbotFactory factory(bot, master->GetLevel(), ITEM_QUALITY_LEGENDARY, mixedGearScore);
                 factory.Randomize(false);
                 return "成功, 装备分数限制: " + std::to_string(mixedGearScore / PlayerbotAI::GetItemScoreMultiplier(ItemQualities(ITEM_QUALITY_EPIC))) +
                        "(for epic)";
             }
             else if (cmd.starts_with("init=") && sscanf(cmd.c_str(), "init=%d", &gs) != -1)
             {
                 ...
                 factory.Randomize(false);
                 return "成功, 装备分数限制: " + std::to_string(gs / PlayerbotAI::GetItemScoreMultiplier(ItemQualities(ITEM_QUALITY_EPIC))) + "(for epic)";
             }
         }
 ...
 std::vector<std::string> PlayerbotHolder::HandlePlayerbotCommand(char const* args, Player* master)
 {
     ...
     if (!*args)
     {
         messages.push_back("用法: list/reload/tweak/self 或 add/init/remove 玩家名称\n");
         messages.push_back("用法: addclass 职业名称");
         return messages;
     }
     ...
     if (!cmd)
     {
         messages.push_back("用法: list/reload/tweak/self 或 add/init/remove 玩家名称 或 addclass 职业名称");
         return messages;
     }
     ...
     if (!strncmp(cmd, "initself=", 9))
     {
         ...
         if (sscanf(cmd, "initself=%d", &gs) != -1)
         {
             if (master->GetSession()->GetSecurity() >= SEC_GAMEMASTER)
             {
                 ...
                 factory.Randomize(false);
                 messages.push_back("自身初始化成功, 装备分数 = " + std::to_string(gs));
                 return messages;
             }
             else
             {
     ...
     if (!strcmp(cmd, "tweak"))
     {
         ...
         messages.push_back("设置调整值为 " + std::to_string(sPlayerbotAIConfig->tweakValue));
         return messages;
     }
     ...
     if (!strcmp(cmd, "addclass"))
     {
         ...
         {
             messages.push_back("你没有权限通过addclass命令创建机器人");
             return messages;
         }
         if (!charname)
         {
             messages.push_back(
                 "addclass: 无效的职业名称(战士/圣骑士/猎人/潜行者/牧师/萨满/法师/术士/德鲁伊/死亡骑士)");
             return messages;
         }
         uint8 claz;
